package com.samyotech.laundrymitra.ui.activity.chat;
...
public class ChatToKonsumenActivity extends AppCompatActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener {
    private final String TAG = ChatToKonsumenActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = ChatToKonsumenActivity.this;
    ...
    public void getComment() {
        ...
        final ANRequest request =
                AndroidNetworking.get(Consts.API_URL + Consts.hubungikonsumen + "?konsumen_id=" + getIntent().getStringExtra("TO_USER_ID") + "&" + "user_id=" + userDTO.getUser_id())
...
//                        .addQueryParameter("otp", otpSms)
    ...
    public void doComment() {
        ...
        try {
            ...
            filePart = MultipartBody.Part.createFormData("media",
                    fileImage.getName(), RequestBody.create(MediaType.parse("image/*"), file));
        } catch (NullPointerException e) {
//            String path = "android.resource://" + getPackageName() + "/" + R.drawable.ic_action_action_search;
//            File fileNew = new File(path);
//            filePart = MultipartBody.Part.createFormData("media",
//                    fileNew.getName(), RequestBody.create(MediaType.parse("file/*"), fileNew));
        ...
        RequestBody messageId = RequestBody.create(MediaType.parse("text/plain"),
                getIntent().getStringExtra("TO_USER_ID"));

        final ProgressDialog progressDialog = new ProgressDialog(this, R.style.CustomAlertDialog);
        progressDialog.setMessage("loading");
        progressDialog.show();
        Log.e("TAG", "uses " + userDTO.getUser_id() + " " + getIntent().getStringExtra("ID") + " " + messageId);
        api.sendMessageToKonsumen(userId, messageId, message, filePart).enqueue(new Callback<ResponseOther>() {
            @Override
            public void onResponse(Call<ResponseOther> call, retrofit2.Response<ResponseOther> response) {
                progressDialog.dismiss();
                edittextMessage.setText("");

                Log.e("TAG", "berhasil kirim chat " + new Gson().toJson(response));
                if (response.isSuccessful()) {
                    if (response.body().isStatus()) {
                        ProjectUtils.showToast(getBaseContext(), "Berhasil Kirim");
                        getComment();
            ...
            public void onFailure(Call<ResponseOther> call, Throwable t) {
                ...
                getComment();
                edittextMessage.setText("");
                Log.e("TAG", "Gagal kirim chat" + t.getMessage());
                //ProjectUtils.showToast(getBaseContext(), "Gagal Kirim");
            }
        });
